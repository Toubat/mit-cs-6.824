...
// func TestInitialization2A(t *testing.T) {
// 	servers := 3
// 	cfg := make_config(t, servers, false)
// 	defer cfg.cleanup()

// 	cfg.begin("Test (2A): initialization")
...
func TestLargerElection2A(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
...
func TestBasicAgree2B(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		DPrintf("Test (2B): check commit for index %v ...", index)
		nd, _ := cfg.nCommitted(index)
		DPrintf("Test (2B): check commit for index %v; nd %v", index, nd)

		if nd > 0 {
			t.Fatalf("some have committed before Start()")
		}

		DPrintf("Test (2B): start index %v ...", index)
